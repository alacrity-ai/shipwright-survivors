...
import { QuestBreakdownWindow } from '@/game/quests/ui/QuestGlobalBreakdownWindow';
import { MissionMutatorMenu } from '@/game/missions/ui/MissionMutatorMenu';
...
export class GalaxyMapSceneManager {
  ...
  private questBreakdownWindow: QuestBreakdownWindow;
  private missionMutatorMenu: MissionMutatorMenu;
  ...
  constructor(
    ...
    this.questBreakdownWindow = new QuestBreakdownWindow();
    this.missionMutatorMenu = new MissionMutatorMenu(inputManager);
    ...
    this.buttons = [
      {
        ...
        onClick: () => {
          ...
          if (selected) {
            // If a mission is selected, deselect instead of leaving the screen
            ...
            // No mission selected → behave as before (return to title)
  ...
  // Gross Helper method because of bad mission naming to assist in displaying the quest breakdown
  private getMissionByActualMissionName(missionId: string) {
    if (missionId === 'mission_002') {
      return 'mission1';
    } else if (missionId === 'mission_003_00') {
      return 'mission2';
    } else if (missionId === 'mission_004_00') {
      return 'mission3';
    } else if (missionId === 'mission_005_00') {
      return 'mission4';
    } else if (missionId === 'mission_006_00') {
      return 'mission5';
    } else {
      return missionId;
    }
  ...
  async start() {
    ...
    // audioManager.play('assets/sounds/sfx/voice/select_mission.wav', 'sfx', { maxSimultaneous: 1 });
  ...
  private pointInScaledRect(px: number, py: number, rect: { x: number; y: number; width: number; height: number }, scale: number): boolean {
  ...
  private isPointerOverAnyUI(px: number, py: number, scale: number): boolean {
    for (const btn of this.buttons) {
      if (this.pointInScaledRect(px, py, btn, scale)) return true;
    }
    if (this.selectedLocationLaunchButton && this.pointInScaledRect(px, py, this.selectedLocationLaunchButton, scale)) {
      return true;
    }
    return false;
  }
  ...
  private update = (dt: number) => {
    ...
    // Capture selection before controller processes input
    const selectedBefore = this.galaxyMapController.getSelectedLocation();
    ...
    // Let the controller update (may react to input)
    this.galaxyMapController.update();

    // Prevent "click empty space to zoom out" behavior:
    // If we had a selection, user clicked, cursor is not over a planet, and not over UI → restore selection.
    const hoveredAfter = this.galaxyMapController.getHoveredLocation();
    const selectedAfter = this.galaxyMapController.getSelectedLocation();
    const clickOnUI = this.isPointerOverAnyUI(x, y, scale);

    if (clicked && selectedBefore && !hoveredAfter && !clickOnUI && !selectedAfter) {
      // Re-instate the prior selection to nullify controller’s empty-click deselect.
      this.galaxyMapController.setSelectedLocation(selectedBefore);
    }
    ...
    if (missionId) {
      if (missionId !== this.currentMissionId) {
        ...
        // Set questbreakdown menu
        this.questBreakdownWindow.filterByMission(this.getMissionByActualMissionName(missionId));
        this.missionMutatorMenu.openMenu();
      ...
      this.missionMutatorMenu.closeMenu();
  ...
  private render = () => {
    ...
    // Quest breakdown menu
    if (selectedLocation) {
      this.questBreakdownWindow.render(uiCtx, scale, 100);
      this.missionMutatorMenu.render();
    }
