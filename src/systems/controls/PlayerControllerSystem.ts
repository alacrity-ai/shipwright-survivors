...
export class PlayerControllerSystem {
  ...
  private getMovementIntent(dt: number): MovementIntent {
    ...
    const afterburnerReq = this.input.isActionPressed('afterburner');
    const thrustBase     = usingPad
      ? leftMag > 0.1
      : this.input.isActionPressed('thrustForward');

    // “Afterburner implies thrust” unless the pilot is braking.
    let thrustForward    = thrustBase || (afterburnerReq && !brake);
    if (brake) thrustForward = false;           // Brake cancels thrust & afterburner

    const afterburner    = afterburnerReq && thrustForward;
