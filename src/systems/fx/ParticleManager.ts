...
export class ParticleManager {
  ...
  emitBurst(origin: { x: number; y: number }, count: number, options: ParticleOptions = {}): void {
    for (let i = 0; i < count; i++) this.spawnParticle(origin, options);
  }
  ...
  emitContinuous(origin: { x: number; y: number }, dt: number, ratePerSecond: number, options: ParticleOptions = {}): void {
    ...
    for (let i = 0; i < particlesToEmit; i++) this.spawnParticle(origin, options);
  }
  ...
  update(dt: number): void {
    ...
    for (let readIndex = 0; readIndex < this.activeParticles.length; readIndex++) {
      ...
      if (particle.fadeMode === 'delayed') {
        particle.renderAlpha = lifeRatio >= fadeThreshold ? 1.0 : lifeRatio * invFadeThreshold;
      } else {
        particle.renderAlpha = lifeRatio;
      }
  ...
  public collectVisibleParticles(camera: Camera): Particle[] {
    const rawBounds = camera.getViewportBounds();
    const floorBounds = {
      x: Math.floor(rawBounds.x),
      y: Math.floor(rawBounds.y),
      width: Math.floor(rawBounds.width),
      height: Math.floor(rawBounds.height),
    };

    const boundsChanged =
      !this.lastCameraBounds ||
      floorBounds.x !== this.lastCameraBounds.x ||
      floorBounds.y !== this.lastCameraBounds.y ||
      floorBounds.width !== this.lastCameraBounds.width ||
      floorBounds.height !== this.lastCameraBounds.height;

    if (!this.particlesDirty && !boundsChanged) {
      return this.cachedVisibleParticles;
    }

    const left = floorBounds.x - this.CULL_PADDING;
    const right = floorBounds.x + floorBounds.width + this.CULL_PADDING;
    const top = floorBounds.y - this.CULL_PADDING;
    const bottom = floorBounds.y + floorBounds.height + this.CULL_PADDING;

    this.cachedVisibleParticles = this.activeParticles.filter(p => {
      const { x, y, size } = p;
      return (
        x + size >= left &&
        x - size <= right &&
    ...
    this.lastCameraBounds = floorBounds;
