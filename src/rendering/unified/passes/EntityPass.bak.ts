...
export class EntityPass {
  ...
  render(ships: Ship[], lightTexture: WebGLTexture, camera: Camera): void {
    ...
    const halfW = camera.getViewportWidth() / (2 * zoom);
    const halfH = camera.getViewportHeight() / (2 * zoom);

    const projectionMatrix = createOrthographicMatrix(-halfW, halfW, halfH, -halfH);
    const viewMatrix = createTranslationMatrix(-camera.getPosition().x, -camera.getPosition().y);
    ...
    for (const ship of ships) {
      ...
      for (const [coord, block] of ship.getAllBlocks()) {
        ...
        if (sprite.overlay && this.inputManager) {
          ...
          gl.uniform2f(this.uniforms.uBlockPosition, blockLocalX, blockLocalY); // ← ADD THIS LINE
